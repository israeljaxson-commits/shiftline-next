...
function TimelineColumn({ title, items }: { title: string; items: readonly { title: string; text: string }[] }) {
  return (
    <div className="timeline-column">
      <p className="mb-8 font-mono text-[10.5px] font-semibold uppercase tracking-[0.16em] text-charcoal/55">{title}</p>
      <ol className="timeline-line relative space-y-6 border-l border-charcoal/18 pl-8">
        {items.map((item, index) => (
          <li key={item.title} className="timeline-item relative rounded-xl border border-charcoal/10 bg-white/55 p-5 backdrop-blur-sm">
            <span
              className={`absolute -left-[38px] top-0 h-3 w-3 rounded-full ${index === 0 ? "bg-emerald-bright" : "border border-charcoal/30 bg-white"}`}
            />
            <p className="mb-1.5 font-display text-[1.52rem] leading-[1.02]">{item.title}</p>
